...
int contact_dila(Contact* contact);//扩容函数

void data_input(Contact* contact)//将文件中的数据导入数组中
{
	FILE* pf = fopen("test.data", "rb");
	if (pf == NULL)
	{
		perror("data_input:fopen");
		return;
	}
	Peoinfor tmp = { 0 };
	while ((fread(&tmp, sizeof(Peoinfor), 1, pf)) == 1)
	{
...
{
	Peoinfor* ptr = (Peoinfor*)realloc(contact->num, sizeof(Peoinfor) * (contact->capacity + MAX_AD));
	if (ptr == NULL)
	{
		return 0;
	}
...
{
	memset(contact->num, 0, sizeof(Peoinfor) * contact->sz);
	contact->sz = 0;
	printf("清除成功\n");
}
...
void contact_memory(Contact* contact)
{
	...
	for (i = 0; i < contact->sz; i++)
	{
		fwrite(contact->num + i, sizeof(Peoinfor), 1, pf);
	}
	fclose(pf);
	pf = NULL;
}
